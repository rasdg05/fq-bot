...
describe("Card viva — el congelado es sagrado", () => {
  ...
  it("los dos lados van nombrados, con su pago y su barra", () => {
    ...
    expect(screen.getAllByTestId("live-barra")).toHaveLength(2);
  });
...
describe("Presupuesto de la card viva", () => {
   ...
   */
  it("mantiene las cuatro filas que caben junto al esqueleto del feed", () => {
    render(<MarketCard market={mercadoVivo()} onOpen={() => {}} />);
    const card = screen.getByTestId("market-card");
    // reloj · activo+precio · pills · pie. Una fila de más es la forma en que
    // se pasa del alto de la card normal sin darse cuenta
    expect(card.querySelector("div > div")?.children.length).toBe(4);
  });

  it("el porcentaje conserva su escala después de `twMerge` (R-004)", () => {
    render(<MarketCard market={mercadoVivo()} onOpen={() => {}} />);
    // `twMerge` mete `text-prob-sm` y `text-text` en el mismo grupo `text-*` y
    // se queda con el último: con el orden al revés el porcentaje salía en
    // 16 px y sólo lo cazaba el navegador. Esto lo caza aquí
    for (const nodo of screen.getAllByTestId("live-pill")) {
      const prob = nodo.querySelector('[data-role="probability"]');
      expect(prob?.className).toContain("text-prob-sm");
    }
  });
});
