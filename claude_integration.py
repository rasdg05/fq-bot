...
def build_niveles_prompt(s):
    ...
    return (
        ...
        "DERIVADOS:\n{}\n\n"
       "----\n"
        "Tu trabajo en 4 bloques:\n\n"
        "VALIDACION: Confirma o cuestiona el plan. El trigger es solido? "
        "La zona tiene sentido dado el contexto de velas recientes?\n\n"
        "SL: El SL propuesto esta anclado a estructura real o a un nivel fragil? "
        "Si ves un nivel mejor (soporte/resistencia estructural mas claro), "
        "daslo en numero exacto con razon de 1 linea.\n\n"
        "CONTEXTO DE RIESGO: Revisa funding, OI, L/S ratio y walls. "
        "Si no hay datos externos disponibles, dicho explicitamente y explica "
        "que riesgo implica operar sin esa informacion en sesion NY donde "
        "los derivados mueven el spot. "
        "RSI en zona 40-50 sin cruzar 50 = momentum no confirmado, mencionalo. "
        "Volumen colapsado vs previa = indecision, mencionalo. "
        "Walls limpias sin bid/ask hostil = neutro, no alcista, aclaralo.\n\n"
        "DECISION: Si/No entradas con este setup ahora mismo. "
        "Una linea, directo. Si no, que condicion adicional esperarias ver.\n\n"
        "Maximo 5 parrafos. Numeros exactos cuando los tengas."
    ).format(
        s.get("price", 0), s.get("session", "?"), s.get("w_clock", 0),
        s.get("bias", "?"), s.get("bias_score", 0),
