...
def build_help_vip():
    return "\n".join([
        ...
        lux_item("/lectura", "Analisis on-demand · SOL"),
        lux_item("/btc", "Analisis on-demand · BTC"),
        lux_item("/eth", "Analisis on-demand · ETH"),
...
def build_help_admin():
    return "\n".join([
        ...
        "",
        lux_block("Cliente:"),
        "   /status /lectura /btc /eth /miestado /renovar /about /legal",
        "",
        lux_block("Alias:"),
        "   /analisis /niveles /pspace /claude /ia → /lectura",
        "   + arg SOL|BTC|ETH (ej. /analisis ETH), default SOL",
        "   /btc = /analisis BTC · /eth = /analisis ETH (atajos BotFather)",
        "   /sesion /macro → /status",
        "",
        lux_block("Admin:"),
