...
# Capa 3 (order-flow) -> motor 1 en la senal VIP. Dos flags INDEPENDIENTES, ambos
# default OFF (senal byte-identica cuando off). Requieren el colector (FQ_CVD_FILTER):
#   FQ_CVD_VIP_CONVICTION -> badge "◆ ORDER-FLOW CONFIRMADO" (marca el HECHO).
#   FQ_CVD_BOOST_TIER     -> la confirmada SUBE +1 tier de conviccion y de size. Es lo
#                            que vuelve el +0.34R del backtest en producto (la senal
#                            con order-flow a favor PESA mas, no solo se etiqueta).
# No cortan cadencia (todas las senales siguen saliendo) y se miden forward en paralelo.
...
def _cvd_vip_kwargs(report, ts, pair):
    ...
    capa 3 esta on -> senal byte-identica. Defensivo: JAMAS rompe el broadcast (sin
    data/ccy/ventana -> sin badge/boost)."""
    if not (CVD_VIP_CONVICTION or CVD_BOOST_TIER):
        return {}
    try:
        import motor_paper
