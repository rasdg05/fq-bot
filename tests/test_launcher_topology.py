...
def test_cvd_off_por_defecto_y_no_critico(monkeypatch):
    monkeypatch.delenv("TELEGRAM_TOKEN_PUBLIC", raising=False)
    monkeypatch.setenv("TELEGRAM_TOKEN", "vip-tok")
    monkeypatch.delenv("FQ_CVD_COLLECT", raising=False)
    assert launcher._cvd_collect_enabled() is False
    assert "cvd-collect" not in [n for n, *_ in launcher._bots()]
    monkeypatch.setenv("FQ_CVD_COLLECT", "1")
    bots = launcher._bots()
    assert "cvd-collect" in [n for n, *_ in bots]
    crit = {n: c for n, _cmd, c in bots}
