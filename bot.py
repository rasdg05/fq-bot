...
INTRA_CANDLE_MINUTES   = 12    # Evaluar a los 12m de la vela (intra-vela)
...
if btc_chg > 0.0008 and eth_chg > 0.0008:
result[“passed”] = True
result[“direction”] = “long”
elif btc_chg < -0.0008 and eth_chg < -0.0008:
result[“passed”] = True
...
return {“passed”: len(masses) >= 2, “count”: len(masses), “masses”: masses}
...
last_intra_eval = None  # Para no evaluar intra-vela mas de 1 vez por vela
...
while True:
    try:
        ...
        # Minutos transcurridos desde inicio de la vela actual
        elapsed_min = (now_utc - current_ts.to_pydatetime().replace(tzinfo=timezone.utc)).seconds // 60

        is_new_candle  = (last_candle_ts is None or current_ts > last_candle_ts)
        is_intra_ready = (elapsed_min >= INTRA_CANDLE_MINUTES and
                          last_intra_eval != current_ts)

        if is_new_candle:
            last_candle_ts = current_ts
            last_intra_eval = None  # reset intra-eval para nueva vela
            BOT_STATE["last_candle_ts"] = current_ts
            log.info(f"Vela cerrada: {current_ts}")

        should_eval = is_new_candle or is_intra_ready

        if should_eval:
            if is_intra_ready and not is_new_candle:
                log.info(f"Evaluacion intra-vela ({elapsed_min}m de 15m)")
                last_intra_eval = current_ts

            in_cooldown = last_signal_ts and (now_utc - last_signal_ts) < cooldown
            if in_cooldown:
                remaining = cooldown - (now_utc - last_signal_ts)
                log.info(f"Cooldown activo: {remaining}")
            else:
                if evaluate_setup(exchange):
                    last_signal_ts = now_utc
                    BOT_STATE["last_signal_ts"] = now_utc
